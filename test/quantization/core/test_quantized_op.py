...
class TestQuantizedConv(TestCase):
    ...
    def _test_qconv_impl(
        ...
        if post_op == 'sigmoid_mul' or post_op == 'gelu_default' or post_op == 'gelu_tanh':
            if fp32_output:
    ...
    def test_qconv2d_fp32_output(
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = torch.ops.quantized.conv2d
                qconv_fp32_output = torch.ops.quantized.conv2d_fp32_output
                qconv_prepack = torch.ops.quantized.conv2d_prepack
                conv_op = torch.nn.Conv2d(
                    input_channels,
                    output_channels,
                ...
                act_qdtypes = [torch.quint8]
                for X_qdtype in act_qdtypes:
                    print("---- first test step ----", flush=True)
                    self._test_qconv_impl(
                        qconv, qconv_prepack, conv_op, batch_size,
                        input_channels_per_group, (height, width),
                        output_channels_per_group, groups, kernels, strides, pads, None,
                    ...
                    self._test_qconv_impl(
                        ...
                        dilations, X_scale, X_zero_point, W_scale, W_zero_point,
                        Y_scale, Y_zero_point, use_bias, "none", use_channelwise, False,
                        input_dtype=X_qdtype, output_dtype=X_qdtype, fp32_output=True)
    ...
    @skipIfNoONEDNN
    def test_qconv2d_fp32_output_relu(
        self,
    ):
        batch_size = 3
        groups_list = [1, 10]
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = torch.ops.quantized.conv2d_relu
                qconv_fp32_output = torch.ops.quantized.conv2d_relu_fp32_output
                qconv_prepack = torch.ops.quantized.conv2d_prepack
                conv_op = torch.nn.Conv2d(
                    input_channels,
                    output_channels,
    ...
    @skipIfNoONEDNN
    def test_qconv2d_fp32_output_add(self):
        batch_size = 3
        groups_list = [1, 10]
        input_channels_per_group = 2
        output_channels_per_group = 2
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = torch.ops.quantized.conv2d_add
                qconv_fp32_output = torch.ops.quantized.conv2d_add_fp32_output
                qconv_prepack = torch.ops.quantized.conv2d_prepack
                conv_op = torch.nn.Conv2d(
                    input_channels,
                    output_channels,
    ...
    @skipIfNoONEDNN
    def test_qconv2d_fp32_output_add_relu(self):
        batch_size = 3
        height = 10
        width = 10
        groups_list = [1, 10]
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = torch.ops.quantized.conv2d_add_relu
                qconv_fp32_output = torch.ops.quantized.conv2d_add_relu_fp32_output
                qconv_prepack = torch.ops.quantized.conv2d_prepack
                conv_op = torch.nn.Conv2d(
                    input_channels,
                    output_channels,
    ...
    def test_qconv2d_gelu(
        ...
        for groups, use_bias, use_channelwise, X2_zero_point in options:
            with override_quantized_engine('onednn'):
                ...
                qconv = torch.ops.quantized.conv2d_gelu
                qconv_fp32_output = torch.ops.quantized.conv2d_gelu_fp32_output
                qconv_prepack = torch.ops.quantized.conv2d_prepack
                conv_op = torch.nn.Conv2d(
                    input_channels,
                    output_channels,
