...
class TestSelectAlgorithm(BaseTestSelectAlgorithm):
    ...
    def test_linear_silu_linear_mul(self, batch_size, in_features, out_features, bias_gate, bias_up, dtype):
        ...
        with verify(dtype) as (atol, rtol), torch.autocast(device_type="cpu"), torch.no_grad():
            self.common(mod, (v,), atol=atol, rtol=rtol)
