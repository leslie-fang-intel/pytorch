...
if torch._C._has_mkldnn:
    ...
    def _is_valid_computation_mlp_silu_mul_fusion():
        def fn(match):
            computation_nodes = filter_nodes(match.nodes, mkldnn._linear_pointwise.default)
            ...
            if computation_nodes[1].args[1] == wgt:
                return False
            if (
                wgt.meta.get("val").dtype != torch.bfloat16
                or computation_nodes[0].args[1].meta.get("val").dtype != torch.bfloat16
            ):
                # TODO: Extending to support dtype other than bfloat16
                return False
            wgt_size = computation_nodes[0].args[1].meta.get("val").size()
            # Check for same weight size
            if computation_nodes[1].args[1].meta.get("val").size() != wgt_size:
                return False
