...
def _is_valid_quantized_conv_binary_optimization_pattern(binary_unary_attr):
    # Check if it's a valid Conv Binary Pattern:
    # * qconv2d_pointwise should only has one users
    # * Extra input of binary node comes from dequant pattern
    # * Extra input of binary node should has only one user or all of the other
    #   users are ancestor_node of conv node
    def fn(match):
        output_dtype = match.output_node().meta.get("val").dtype
        ...
        if len(qconv2d_node_after_weight_prepack.users) != 1:
            return False
        binary_node = list(qconv2d_node_after_weight_prepack.users)[0]
        ...
        if binary_unary_attr and binary_unary_attr.binary_op_name == "sum":
            # If the extra input of add node can't be inplace changed,
            # we shouldn't use the post op sum fusion.

            from .mkldnn_fusion import _is_ancestor_node

            _skip_check_user_node = (
                extra_input_node.args[0].args[0]
                if output_dtype == torch.uint8
                else binary_node
            )
            _check_node = (
                _skip_check_user_node.args[0]
                if output_dtype == torch.uint8
            ...
            if len(_check_node.users) > 1 and not all(
                (
                    user == _skip_check_user_node
                    or _is_ancestor_node(qconv2d_node_after_weight_prepack, user)
                )
                for user in list(_check_node.users)
            ):
                return False
...
def _register_quantized_conv_binary_lowering(
    ...
    def qconv_binary(match: Match, *args, **kwargs):
        output_dtype = match.output_node().meta.get("val").dtype
        assert output_dtype in [torch.uint8, torch.float32, torch.bfloat16]
        assert binary_unary_attr.binary_op_name in ["sum", "binary_add"]
        x, x_scale, x_zp = kwargs["x"], kwargs["x_scale"], kwargs["x_zp"]
        accum = (
            kwargs["accum"]
            if (output_dtype == torch.uint8 and binary_unary_attr.binary_op_name == "sum")
        ...
        counters["inductor"][
            f"qconv2d_{binary_unary_attr.binary_op_name}_matcher_count"
        ] += 1
        counters["inductor"][
            f"qconv2d_{binary_unary_attr.binary_op_name}_matcher_nodes"
        ] += len(match.nodes)
        return L[computation_op](*computation_args)
...
def _register_quantization_binary_fusion():
    ...
    def _register_quantization_post_op_sum_unary():
        def _register_quantization_post_op_sum_unary_helper(
            int8_mixed_bf16_with_inplace_add,
        ):
            # Priority 1 to match: QConv2d Binary or Binary-Unary pattern with int8 output
            #    QConv2d       dq
            #            add
            #       optional(to_bf16) if int8_mixed_bf16_with_inplace_add
            #       optional(relu) if register unary post op
            #       optional(to_fp32) if int8_mixed_bf16_with_inplace_add
            #             q
            # Fuse this pattern to QConv2d_sum_(relu)
            ...
            #       optional(to_bf16) if int8_mixed_bf16_with_inplace_add
            #            relu
            # Fuse this pattern to QConv2d_sum_relu
            ...
            for (
                ...
                _register_quantized_conv_binary_lowering(
                    patterns,
                    0 if int8_mixed_bf16_with_inplace_add else 2,  # pass_number
                    torch.ops.onednn.qconv2d_pointwise.binary,  # computation_op
                    binary_unary_attr,  # binary_unary_attr
                )
            ...
            #            add
            #       optional(to_bf16) if int8_mixed_bf16_with_inplace_add
            # Fuse this pattern to QConv2d_sum
            binary_replace_float_out_patterns = {
                BinaryUnaryAttr(
                    "sum", 1.0, "none", [], ""
                ): generate_pattern_with_binary(
            ...
            for (
                ...
                _register_quantized_conv_binary_lowering(
                    patterns,
                    1 if int8_mixed_bf16_with_inplace_add else 3,  # pass_number
                    torch.ops.onednn.qconv2d_pointwise.binary,  # computation_op
                    binary_unary_attr,  # binary_unary_attr
                )

        _register_quantization_post_op_sum_unary_helper(
            int8_mixed_bf16_with_inplace_add=False
        )
        # For int8_mixed_bf16 and with inplace_add,
        # there are additional dtype conversion nodes at the output of the 'add' operation
        # when first parameter of inplace add is with data type bfloat16 and second parameter
        # of inplace add is with data type float32.
        _register_quantization_post_op_sum_unary_helper(
            int8_mixed_bf16_with_inplace_add=True
        )
    
    def _register_quantization_post_op_binary_add_unary():
            # Priority 1 to match: QConv2d Binary or Binary-Unary pattern with int8 output
            #    QConv2d       
            #            add
            #       optional(relu) if register unary post op
            #       optional(to_fp32)
            #             q
            # Fuse this pattern to QConv2d_binary_add_(relu)
            ...
            binary_replace_patterns = {
                ...
                # Int8-mixed-bf16, inplace add, extra_input of add is float32
                BinaryUnaryAttr(
                    "binary_add", 1.0, "none", [], ""
                ): generate_pattern_with_output_quant(
                    generate_pattern_with_binary(
            ...
            #            add
            #            relu
            # Fuse this pattern to QConv2d_binary_add_relu
            binary_replace_float_out_patterns = {
                BinaryUnaryAttr(
                    "binary_add", 1.0, "relu", [], ""
                ): generate_pattern_with_unary(
    ...
    _register_quantization_post_op_sum_unary()
    _register_quantization_post_op_binary_add_unary()
