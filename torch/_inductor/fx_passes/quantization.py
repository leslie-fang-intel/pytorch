...
import torch
from ..ir import QConv, QConvPointWisePT2E, TensorBox, QConvPointWiseBinaryPT2E
...
def _register_quantized_conv_relu_lowering_pt2e(pattern):
    @register_lowering_pattern(pattern)
    def qconv(match: Match, *args, **kwargs):
        x, x_scale, x_zp = kwargs["x"], kwargs["x_scale"], kwargs["x_zp"]
        b, stride, padding, dilation = (
        ...
        pattern_match_count += 1
        print(
            "---- matched the qconv_relu pattern ----: {}".format(pattern_match_count), flush=True
        )
...
def _register_quantized_conv_add_lowering_pt2e(pattern):
    @register_lowering_pattern(pattern)
    def qconv(match: Match, *args, **kwargs):
        x, x_scale, x_zp = kwargs["x"], kwargs["x_scale"], kwargs["x_zp"]
        ...
        pattern_match_count += 1
        print(
            "---- matched the qconv_add pattern ----: {}".format(pattern_match_count), flush=True
        )

        weight_shape = packed_weight.get_size()
        dim = len(weight_shape) - 2
        return TensorBox.create(
                QConvPointWiseBinaryPT2E.create(
                dim,
                x,
                x_scale,
                x_zp,
