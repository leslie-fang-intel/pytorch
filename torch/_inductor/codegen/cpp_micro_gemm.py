...
{%- if use_cached_dequantized_B %}
    ...
    auto load_dequantized_B = [&](int n) {
        ...
        for (int idx=0; idx<(K * {{block_n}}); idx+=32) {
            if ((idx + 32) < (K * {{block_n}})) {
                auto b_int8 = at::vec::Vectorized<int8_t>::loadu(const_cast<{{input2_t}}*>(B) + idx, static_cast<int64_t>(32));
                auto b_bf16 = at::vec::convert<{{input_t}}>(b_int8);
                b_bf16.store(dequantized_B_buf + idx);
            } else {
                for (int idx2 = idx; idx2 < (K * {{block_n}}); idx2 ++) {
                   dequantized_B_buf[idx2] =  static_cast<{{input_t}}>(*(const_cast<{{input2_t}}*>(B) + idx2));
                }
            }
        }
    };
