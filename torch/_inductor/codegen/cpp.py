...
class OuterLoopFusedSchedulerNode(FusedSchedulerNode):
    ...
    def fuse(  # type: ignore[override]
        ...
        if any(
            type(node) is OuterLoopFusedSchedulerNode
            for node in (node1, node2)
        ):
            return cls(
                node1.scheduler,
                (list(node1.get_outer_nodes()) if type(node1) is OuterLoopFusedSchedulerNode else [node1,]) +
                (list(node2.get_outer_nodes()) if type(node2) is OuterLoopFusedSchedulerNode else [node2,]),
...
class CppScheduling(BaseScheduling):
    ...
    def _get_outer_loop_fusion_depth(self, node1, node2):
        ...
        if not (
            ...
            and all(
                type(node) in (OuterLoopFusedSchedulerNode, FusedSchedulerNode, SchedulerNode)
                for node in (node1, node2)
            )
        ):
        ...
        if (
            ...
            if any(
                ...
                _compare_node = node1 if type(node1) is OuterLoopFusedSchedulerNode else node2
    ...
    def codegen_node(self, node: Union[OuterLoopFusedSchedulerNode, FusedSchedulerNode, SchedulerNode]):
        """
            ...
            def _check_loop_level_attr(
                ...
                def _get_parallel_depth(_cpp_proxy_kernel):
                    return _cpp_proxy_kernel.decide_parallel_depth(
                        _cpp_proxy_kernel.call_ranges[: _cpp_proxy_kernel.loop_nest.max_parallel_depth()],
                        parallel_num_threads()
                    )
                ...
                for idx in range(len(_cpp_kernel_proxy_list) - 1):
                    ...
                    if _get_parallel_depth(_cpp_kernel_proxy_list[idx]) != _get_parallel_depth(_cpp_kernel_proxy_list[idx + 1]):
