...
at::Tensor PackedConvWeightsOnednn<kSpatialDim>::apply_impl(
  ...
  if (transpose()) {
    ...
    c10::call_once(*cache_initialized_flag, [&](){
        ...
        auto w_dims = kernel_size;
        if (w_dims[0] == 1024
          && w_dims[1] == 512
          && w_dims[2] == 1
          && w_dims[3] == 1) {
          std::cout<<"---- hit hot spot in 1.x conv calculation-----"<<std::endl;
        ...
        auto expected_weight_desc = ideep::tensor::desc(params.pd.weights_desc(), groups());
        std::cout<<"---- start check 1.x conv reorder ----"<<std::endl;
        weights = weights.reorder_if_differ_in(expected_weight_desc);
        std::cout<<"---- finish check 1.x conv reorder ----"<<std::endl;
    });
...
static at::Tensor _quantized_convolution_onednn(
  ...
  // std::vector<int32_t> wgt_zero_points(weight_scales.numel(), 0);
  ...
  ideep::zero_point_t dst_zero_points = ideep::zero_point_t(1, output_zero_point);

  // Weight
  auto packed_weight = at::native::itensor_from_mkldnn(weight);


  // packed_weight.set_scale(weights_scales);
  // packed_weight.set_zero_point(wgt_zero_points);
  ...
  if (w_dims[0] == 256
    ...
    && w_dims[2] == 1
    && w_dims[3] == 1) {
    inv_output_scale = 1.0;
    dst_zero_points = ideep::zero_point_t(1, 0);
  }
  ...
  if (w_dims[0] == 2048
    ...
    && w_dims[2] == 1
    && w_dims[3] == 1) {
    // std::cout<<"---- hit hot spot in 2.x conv calculation-----"<<std::endl;
    // std::cout<<"src_scales is: "<<src_scales<<std::endl;
    // std::cout<<"src_zero_points is: "<<src_zero_points<<std::endl;
    // // std::cout<<"weights_scales is: "<<weights_scales<<std::endl;
    // std::cout<<"inv_output_scale is: "<<inv_output_scale<<std::endl;
  ...
  // int num_threads = at::get_num_threads();
  // PrimitiveCacheKey cache_key = std::make_tuple(
  //     act_scale, act_zero_point, src_dims, inv_output_scale, output_zero_point, num_threads, accum_scale, accum_zero_point);
  
  // std::cout<<"---- start check 2.x conv reorder ----"<<std::endl;
  ideep::tensor expected_weight = packed_weight.reorder_if_differ_in(expected_weight_desc);
  // std::cout<<"---- finish check 2.x conv reorder ----"<<std::endl;
