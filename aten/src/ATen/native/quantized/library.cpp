...
TORCH_LIBRARY(quantized, m) {
  ...
  // For Inductor path, before aot-autograd we have a conv accept fp32 activation, int8 prepacked weight
  m.def(TORCH_SELECTIVE_SCHEMA("quantized::conv_prepacked_weight.tensor(Tensor x, Tensor w, Tensor? bias, Tensor qw, Tensor weight_scale, Tensor weight_zero_point, int w_axis, Tensor? q_bias, int[] stride, int[] padding, int[] dilation, bool is_transposed, int[] out_padding, int groups, Tensor x_scale, Tensor x_zero_point) -> Tensor"));
